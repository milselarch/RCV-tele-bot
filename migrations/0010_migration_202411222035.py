...
class Payments(peewee.Model):
    ...
    telegram_payment_charge_id = CharField(default='', max_length=255)
    amount = IntegerField()
...
def forward(old_orm, new_orm):
    old_payments = old_orm['payments']
    payments = new_orm['payments']
    return [
        # Convert datatype of the field payments.telegram_payment_charge_id: TEXT -> VARCHAR(255),
        payments.update({payments.telegram_payment_charge_id: old_payments.telegram_payment_charge_id.cast('CHAR')}).where(old_payments.telegram_payment_charge_id.is_null(False)),
    ]
...
def backward(old_orm, new_orm):
    ...
    return [
        ...
        payments.update({payments.telegram_payment_charge_id: old_payments.telegram_payment_charge_id}).where(old_payments.telegram_payment_charge_id.is_null(False)),
    ]
